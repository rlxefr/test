...
def easy_aim_print(msg: str) -> str:
    return f"[{CYAN}Velocity{RESET}] -> {msg}"

def easy_aim_input(msg: str) -> str:
    prompt = f"[{CYAN}Velocity{RESET}] -> {msg}"
    return input(prompt)
...
def install_process():
    ...
    with open(INSTALLED_FLAG_FILE, 'w') as f:
        f.write("[Velocity] -> Installation successful.\n")
...
# Set the asset directory paths
ASSETS_DIR1 = r'C:\\ProgramData\\Easy Aim\\Assets'
ASSETS_DIR = r'C:\\ProgramData\\Clarity\\Assets'
IMAGES_DIR = os.path.join(ASSETS_DIR, 'Images')

FONT_DIR = r'C:\\ProgramData\\fontai'
...
os.makedirs(FONT_DIR, exist_ok=True)  # Ensure FONT_DIR exists

def easy_aim_print(message):
    return f"[Velocity] {message}"
...
def download_font():
    if not os.path.exists(FONT_PATH):
        print(easy_aim_print("Downloading font..."))
...
class api:
    ...
    def checkinit(self):
        if not self.initialized:
            print("[Velocity] -> initialize first in order to use the functions")
            time.sleep(2)
            os._exit(1)
    ...
    def __do_request(self, post_data):
        try:
            ...
            message = (
                ...
                "Download the file and turn on WARP (not cloudflare) before launching Velocity.\n"
            ...
            webbrowser.open('https://discord.gg/78n4CDc4gD', new=2)
            time.sleep(0.2)
            try:
                console_window = ctypes.windll.kernel32.GetConsoleWindow()
                ctypes.windll.user32.PostMessageW(console_window, 0x10, 0, 0)
...
class FPSOverlay(QWidget):
    def __init__(self):
        ...
        font_id = QFontDatabase.addApplicationFont("C:\\ProgramData\\fontai\\Font.ttf")
        font_family = QFontDatabase.applicationFontFamilies(font_id)[0]
        custom_font = QFont(font_family)

        self.label = QLabel("Velocity | 0 ENEMIES | 000 FPS", self)
    ...
    def update_fps(self):
        self.label.setText(
    f"<span style='color:#0084ff;'>Velocity | FORTNITE | V1.02 | FPS: {int(self.fps)}</span>"
)
...
class MyWindow(QWidget):
    try:
        ...
        RP03EV27S("FortniteTensor.engine", "https://github.com/rlxefr/SomeTens.pt/raw/refs/heads/main/FortniteTensor.engine")
...
# Slots Start
        ...
        image_files = [
            ""	
        ]
        ...
        button_container_layout05.setAlignment(Qt.AlignTop)
        ...
        button_container_layout_slot1.setAlignment(Qt.AlignTop)
        ...
        button_container_layout_slot2.setAlignment(Qt.AlignTop)
        ...
        button_container_layout_slot3.setAlignment(Qt.AlignTop)
        ...
        button_container_layout_slot4.setAlignment(Qt.AlignTop)
        ...
        button_container_layout_slot5.setAlignment(Qt.AlignTop)
        ...
        button_container_layout_slot6.setAlignment(Qt.AlignTop)
        ...
        button_container_layout_flick_key.setAlignment(Qt.AlignTop)
        ...
        button_container_layout08.setAlignment(Qt.AlignTop)
        ...
        try:
            self.font_size = open("C:\\ProgramData\\Easy Aim\\Assets\\size.txt", "r").read()
        except:
            self.font_size = 15
    ...
    def load_modelss(self):
        ...
        for file in default_model_files:
            if 'FortnitePro' in file:
                ...
                default_models[label] = file 
            elif 'FortniteTensor' in file:
                label = "FortniteTensor" + os.path.splitext(file)[1]
                default_models[label] = file
...
class DetectionBox(QWidget):
    ...
    def load_config(self):
        ...
        self.fov_color = QColor(0,0,0,255)
            ...
            for key, slot in zip([Slot1_Keybind, Slot2_Keybind, Slot3_Keybind, Slot4_Keybind, Slot5_Keybind, Slot6_Keybind], range(1, 7)):
                if key is not None:
                    ...
                    if key not in self.key_states:
                        print(f"[Velocity] -> key {key} not found in key_states")
    ...
    def paintEvent(self, event):
        ...
        if CupMode_On:
            ...
            if Show_Crosshair:
                if self.BlueFire():
                    pen_crosshair_ads = QPen(QColor(0, 0, 0, 0), 0.3, Qt.SolidLine)
                ...
                if Show_Detections:
                    for player in self.detected_players:
                        ...
                        if Box_type == "Corner":
                            ...
                            if Use_Hue:
                                hue = int(time.time() * 150) % 360
                                color = QColor.fromHsv(hue, 0, 0, 0)
                                painter.setPen(QPen(color, 2))
                            else:
                            ...
                            if Use_Hue:
                                ...
                                painter.setPen(QPen(QColor(0, 0, 0, int(255 * 0.75)), 2))

                            fill_color = QColor(0, 0, 0, int(255 * 0.25))
...
class PyProtect():
    def main():
        ...
        try:
            LKey2 = open(rf"{current_directory}\key.txt", "r")
            XFC2 = LKey2.read()
            LKey2.close()
        except:
            XFC2 = "N/A"
        ...
        def block_bad_processes():
            ...
            if blacklisted_process_name:
                try:
                    ...
                    print(f"\n[Velocity] -> blacklisted process; {blacklisted_process_name}")
                    time.sleep(1)
                    exit(1)
...
class LoginForm():
    LKey = open(rf"{current_directory}\key.txt", "r")
    XFC = LKey.read()
    LKey.close()

    if XFC == "":
        os.system("cls")
        print("[Velocity] -> enter your license")
        Answer23 = input("\n> ")
    else:
        pass
    ...
    XlOp09_Au7h_4U_L0ve_CMe = api(name = "Velocity",ownerid = "BTo5iNtY1n", secret = "0c251e7e0c12931d14a9b2421f1262d46b30dca41a48ce0df81dd31fd504b318", version = "1.0", hash_to_check = getchecksum()) # ENTER YOUR KEYAUTH DETAILS

    XlOp09_Au7h_4U_L0ve_CMe.init()
    os.system("cls")
    if XFC == "":
        os.system("cls")
        SaveKeyHere = open(rf"{current_directory}\key.txt", 'w')
